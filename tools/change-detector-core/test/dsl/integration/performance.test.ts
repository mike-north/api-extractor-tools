...
async function measureAsyncPerformance<T>(fn: () => Promise<T>): Promise<{ result: T; metrics: PerformanceMetrics }> {
  const memoryBefore = process.memoryUsage().heapUsed
  const startTime = performance.now()
...
describe('DSL Performance Tests', () => {
  describe('Transformation Performance Benchmarks', () => {
    it('should transform 1000 intent rules in < 100ms', () => {
      const intentRules: IntentRule[] = Array.from({ length: 1000 }, (_, i) => ({
        type: 'intent',
        expression: (i % 2 === 0 ? 'breaking removal' : 'safe addition') as IntentExpression,
      ...
      console.log(`Transformed ${result} rules in ${metrics.duration.toFixed(2)}ms`)
    })

    it('should compile 1000 pattern rules in < 150ms', () => {
      const patternRules: PatternRule[] = Array.from({ length: 1000 }, (_, i) => ({
        type: 'pattern',
        template: (i % 3 === 0 ? 'removed {target}' : i % 3 === 1 ? 'added {target}' : 'modified {target}') as PatternTemplate,
        variables: [{ 
          name: 'target', 
      ...
      console.log(`Compiled ${result} patterns in ${metrics.duration.toFixed(2)}ms`)
    })

    it('should decompile 1000 dimensional rules in < 200ms', () => {
      const dimensionalRules: DimensionalRule[] = Array.from({ length: 1000 }, (_, i) => ({
        type: 'dimensional',
      ...
      console.log(`Decompiled ${result} dimensional rules in ${metrics.duration.toFixed(2)}ms`)
    })

    it('should handle round-trip transformations efficiently', () => {
      const testRules: IntentRule[] = Array.from({ length: 100 }, (_, i) => ({
        type: 'intent',
        expression: (['breaking removal', 'safe addition', 'deprecation is patch'][i % 3]) as IntentExpression,
        returns: (['major', 'minor', 'patch'][i % 3]) as ReleaseType,
      }))
      ...
      console.log(`Completed ${result} round-trips in ${metrics.duration.toFixed(2)}ms`)
    })
  })
  ...
  describe('Large Policy Performance', () => {
    it('should build large policies efficiently', () => {
      const { result: policy, metrics } = measurePerformance(() => {
        ...
        for (let i = 0; i < 500; i++) {
          builder = builder.intent(
            (['breaking removal', 'safe addition', 'deprecation is patch'][i % 3]) as IntentExpression,
        ...
        for (let i = 0; i < 300; i++) {
          builder = builder.pattern(
            (['removed {target}', 'added {target}', 'modified {target}'][i % 3]) as PatternTemplate,
            { target: (['export', 'function', 'property'][i % 3]) as ChangeTarget },
            (['major', 'minor', 'patch'][i % 3]) as ReleaseType
          )
        }
      ...
      console.log(`Built policy with ${policy.rules.length} rules in ${metrics.duration.toFixed(2)}ms`)
    })
    ...
    it('should handle policy transformation at scale', () => {
      ...
      const { result, metrics } = measurePerformance(() => {
        ...
        const transformedBuilder = largePolicyBuilder.transform({ targetLevel: 'dimensional' })
        const policy = transformedBuilder.build('Transformed Large Policy', 'minor')
        
        // Verify transformation
        const allDimensional = policy.rules.every(r => r.type === 'dimensional')
      ...
      console.log(`Transformed ${result.policy.rules.length} rules in ${metrics.duration.toFixed(2)}ms`)
    })
  })

  describe('Deeply Nested Pattern Performance', () => {
    it('should handle deeply nested conditional patterns', () => {
      const deeplyNestedPatterns: PatternRule[] = Array.from({ length: 50 }, (_, i) => {
        ...
        const variables = [
          { name: 'target', value: 'export' as ChangeTarget, type: 'target' as const },
          { name: 'pattern', value: 'removed {target}' as PatternTemplate, type: 'pattern' as const },
        ]
      ...
      const { result, metrics } = measurePerformance(() => {
        const results = deeplyNestedPatterns.map(pattern => compilePattern(pattern))
      ...
      console.log(`Processed ${result} nested patterns in ${metrics.duration.toFixed(2)}ms`)
    })

    it('should handle complex dimensional rules with many attributes', () => {
      const complexDimensionalRules: DimensionalRule[] = Array.from({ length: 100 }, (_, i) => ({
        type: 'dimensional',
        target: ['export', 'function', 'property', 'parameter', 'typeParameter'] as ChangeTarget[],
      ...
      const { result, metrics } = measurePerformance(() => {
        const results = complexDimensionalRules.map(rule => decompileToPattern(rule))
        return {
          successCount: results.filter(r => r.success).length,
          avgConfidence: results.filter(r => r.success).reduce((sum, r) => sum + r.confidence, 0) / results.filter(r => r.success).length,
        }
      })
      ...
      console.log(`Processed ${result.successCount} complex rules in ${metrics.duration.toFixed(2)}ms with avg confidence ${result.avgConfidence.toFixed(3)}`)
    })
  })
  ...
  describe('Memory Efficiency', () => {
    it('should not leak memory during repeated transformations', () => {
      ...
      console.log(`Completed ${iterations} iterations with ${memoryGrowth / 1024 / 1024}MB memory growth`)
    })
    ...
    it('should handle large rule collections without excessive memory usage', () => {
        ...
        for (let policyIndex = 0; policyIndex < 50; policyIndex++) {
          ...
          for (let ruleIndex = 0; ruleIndex < 100; ruleIndex++) {
            ...
            if (ruleType === 0) {
              ...
              builder = builder.pattern('added {target}', { target: 'export' }, 'minor')
            } else {
              builder = builder
      ...
      console.log(`Created ${result.length} policies (${result.length * 100} total rules) using ${memoryGrowth / 1024 / 1024}MB`)
    })
  })
  ...
  describe('Concurrent Operations', () => {
    it('should handle concurrent transformations safely', async () => {
      ...
      console.log(`Completed ${result} concurrent transformations in ${metrics.duration.toFixed(2)}ms`)
    })
    ...
    it('should handle concurrent policy building', async () => {
      ...
      console.log(`Built ${result.length} policies concurrently in ${metrics.duration.toFixed(2)}ms`)
    })
  })
  ...
  describe('Edge Case Performance', () => {
    it('should handle malformed rules without significant performance degradation', () => {
      ...
      const malformedRules: any[] = [
        ...
        { type: 'pattern', template: 'invalid {', variables: [], returns: 'major' },
      ...
      console.log(`Processed ${result} malformed rules in ${metrics.duration.toFixed(2)}ms`)
    })
    ...
    it('should handle extremely large individual rules', () => {
      ...
      const largePattern: PatternRule = {
        type: 'pattern',
        template: Array.from({ length: 50 }, (_, i) => `{var${i}}`).join(' and ') as PatternTemplate,
      ...
      console.log(`Processed large pattern with 50 variables in ${metrics.duration.toFixed(2)}ms`)
    })
  })
  ...
  describe('Performance Regression Detection', () => {
    it('should maintain consistent performance across rule types', () => {
      ...
      // Performance should be relatively consistent across types
      const durations = Object.values(results)
      const maxDuration = Math.max(...durations)
      const minDuration = Math.min(...durations)
      
      // Max should not be more than 10x slower than min (relaxed for test stability)
